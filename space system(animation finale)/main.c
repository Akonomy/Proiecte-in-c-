...
#define sphere(x,string) printf("Transform{ children[ Shape { geometry Sphere { radius  "); printf(#x"  ");  printf("} appearance USE   ");printf(#string"  ");printf( "}}]}\n");
 ...
 #define add(x) printf(" "); printf(#x" ");


float x,y,z,xa,a1,ya,xb,a0,yb,xc,a1=0,yc,xd,a0=0,a2=-1,a3=5,yd,adder,adderdens,adderstates,states=0;
int pas=0,az=0,ad;
float x=0,y=1,z=0,upl,upd,dwl,dwd,cateta1,cateta2,degrees,degreesb=0,cadran=0;
  ...
  int i,cp0=0,cp1=0,cp2=0,cp3=0,cp4=0,css=0,random,yrand=0,ysemn_a=0,ysemn_b=0,ysemn_citire,addstates=20;
...
int main()
{
  ...
  //scanf("%f",&adderdens);
  adderdens=1;
 // printf("Alege pentru animatie starile");
 // scanf("%d",&addstates);
  ...
  system("cls");
   ...
   printf ("  Shape { appearance DEF App Appearance { material Material { diffuseColor 0 .23 .31 }}} \n");
    ...
    printf ("  Shape { appearance DEF App4 Appearance { material Material { diffuseColor 1 1 .12 }}} \n");
    printf ("  Shape { appearance DEF App5 Appearance { material Material { diffuseColor 1 .1 .1 }}} \n");
     printf ("  Shape { appearance DEF App6 Appearance { material Material { diffuseColor 0 1 .31 }}} \n");
      printf ("  Shape { appearance DEF App7 Appearance { material Material { diffuseColor 0 0 .9 }}} \n");
       ...
       printf (" DEF ball Transform {children	[Shape {geometry Sphere{ radius 1 } appearance USE App }]}\n ");
       printf ("  Transform {children	[Shape {geometry Sphere{ radius 1.5 } appearance USE App4 }]}\n ");
...
///Coords generator
       ...
       for (int i=0;i<=360/adderdens;i++)
  {
     ...
     yd=cat40;
       ...
       if(ya!=dega && yb!=dega && yc!=dega && xd!=dega && ya!=dega*(-1) && yb!=dega*(-1) && yc!=dega*(-1) && yd!=dega*(-1) )
        {
            printf(" %f %f %f   ",xa,a0,ya);
        // printf(" %f %f %f   ",xb,a1,yb);
          // printf(" %f %f %f  ",xc,a2,yc);
            // printf(" %f %f %f   ",xd,a3,yd);
  ...
  void animate ()
  {
      adderstates=1/addstates;
     printf("DEF	animator PositionInterpolator	{ key [ 0" );
     for(int i=0; i<360; i++)
     {

    states=states+0.002777;
    printf(" ,%f ",states);
     ...
     printf("DEF timmer TimeSensor	{ cycleInterval	5 loop	TRUE } \nROUTE	timmer.fraction_changed	TO animator.set_fraction \nROUTE animator.value_changed TO ball.set_translation");
  }
